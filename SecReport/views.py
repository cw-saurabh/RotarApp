...
def report(request):
    ...
    month = "0" + str(int(currentMonth)-1) if (currentMonth!=1 and currentMonth<11) else ("12" if currentMonth==1 else str(int(currentMonth)-1))
    print("currentMonth"+str(currentMonth),end=" ")
    print("month"+month)

    if request.user.is_club :

        reportId = str(request.user.rotaryId)+"-"+str(datetime.now().year)+"-"+month
        profile = Club.objects.filter(account=request.user).first()

        if Report.objects.filter(reportId=reportId).exists():
            return render(request, 'SecReport/reportFoundResponse.html',{'title':'Reporting','tab':'report','reportId':reportId,'profile':profile})

        else :

            if request.user.rotaryId == 214097 :
                        ...
                        with transaction.atomic():
                            ...
                            for x in gbmlist :
                                gbm = GBM.objects.create(
                                    ...
                                    gbm1 = request.POST[x+"-1"] if request.POST[x+"-1"] else None,
                            ...
                            for x in bodlist :
                                bod = BOD.objects.create(
                                    ...
                                    bod1 = request.POST[x+"-1"] if request.POST[x+"-1"] else None ,
                            ...
                            for x in eventlist :
                                event = Event.objects.create(
                                    club = request.user,
                                    reportId = report,
                                    event0 = request.POST[x+"-0"] if request.POST[x+"-0"] else None,
                            ...
                            for x in futureEventlist :
                                ...
                                club = request.user,
                                reportId = report,
                                futureEvent0 = request.POST[x+"-0"] if request.POST[x+"-0"] else None,
                ...
                return render(request, 'SecReport/deadlineMissed.html',{'title':'Reporting','tab':'report','reportId':reportId,'profile':profile})
